...
plot(u1, u2, xlab = "x coord", ylab = "y coord", cex = 1, pch = 16, asp = 1,
   ...
   xlim = c(0, quad.size), ylim = c(0, quad.size), frame = F, col = "red") # plot point pattern
...
plot(u1, u2, xlab = "x coord", ylab = "y coord", cex = 1, pch = 16, asp = 1,
   ...
   ylim = c(0, quad.size), frame = F, col = "red") # plot point pattern
# Overlay grid onto study area
for(i in 1:length(breaks)){
   for(j in 1:length(breaks)){
      segments(breaks[i], breaks[j], rev(breaks)[i], breaks[j])
...
col = "black", xlim = c(0, max(N)), main = "Frequency of N with mean density (blue)", lwd = 3, frame = F)
