...
# With implicit default and plot browser turned off
str(data <- simDynoccSpatial(ask.plot = FALSE))

## Check a few custom settings

# Large grid
str(data <- simDynoccSpatial(side = 100))


# Longer series and no browsing through plots
str(data <- simDynoccSpatial(nyears = 100, ask.plot = FALSE))

# Dynamic system (high variance between years)
str(data <- simDynoccSpatial(range.phi = c(0, 0.8), range.gamma = c(0, 0.6)))
...
# Strong spatial autocorrelation via a spatially structured covariate
str(data <- simDynoccSpatial(beta.XAC = c(1, 1, 1, 1), beta.Xautolog = c(2, 2)))

# Strong spatial autocorrelation via a spatially structured covariate
# but now effect of that spatial field only on colonization and persistence
str(data <- simDynoccSpatial(beta.XAC = c(0, 1, 1, 0), beta.Xautolog = c(0, 0)))

# Can it ever go extinct ???
str(data <- simDynoccSpatial(side = 20, nyears = 20, mean.psi1 = 0.01, range.phi = c(0.1, 0.1), range.gamma = c(0.1, 0.1), beta.Xautolog = c(0, 0)))

# Can it ever go extinct ???   -------> Yes
set.seed(24)  ###### won't work
str(data <- simDynoccSpatial(side = 20, nyears = 10, mean.psi1 = 0.01, range.phi = c(0.3, 0.3), range.gamma = c(0, 0), beta.Xautolog = c(0, 0)))
